-- 데이터 조작어 (DML => INSERT / UPDATE / DELETE) => 187page
set linesize 250 
set pagesize 25
/*
	INSERT : 데이터 추가
		...
		= ****전체 데이터 등록
			INSERT INTO table_name VALUES(값, 값...) => DEFAULT가 적용이 안 된다
				table_name => 컬럼과 매칭 (DEFAULT 상관 X)

		= 원하는 데이터 등록 (NULL 허용, DEFAULT가 있는 경우)
...
-- 문자열 => ' ', 날짜 => ' ' / SYSDATE
-- DEFAULT가 있는 경우 => 컬럼 설정
-- => INSERT INTO table_name(컬럼...) VALUSE(값...)
--                                                                   'SYSDATE'
...
-- subquery => insert, update, delete에서 사용 가능
...
COMMIT;
*/
-- COMMIT이 없는 경우, 오라클에서는 종료 전까지 사용 가능하지만 자바에서는 인식하지 못 한다
-- 자바는 실제 저장된 메모리에서 읽는다
/*
INSERT INTO student(hakbun,name,kor,eng)
VALUES((
	SELECT NVL(MAX(hakbun)+1,100) 
	FROM student
),'김두한',90,90); => NULL을 ("HR"."STUDENT"."MATH") 안에 삽입할 수 없습니다
*/
/*
