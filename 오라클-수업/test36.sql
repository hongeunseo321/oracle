...
/*
	...
	특징)
		테이블 / 뷰에 연결
		이벤트가 발생하면 자동호출
		INSERT / UPDATE / DELETE 에서만 사용 가능
		로그 파일 / 백업 시에 주로 사용

	예)
		입고 => 재고 자동화
		출고 => 재고 자동화 
		***맛집 찜 => 찜 개수 증가 (맛집 번호는 Function)
		***웹사이트 댓글은 프로시저(Procedure)

	형식)
		CREATE [OR REPLACE] TRIGGER tri_name
		{BEFORE | ARTER} {이벤트 : INSERT | UPDATE | DELETE} ON 어떤 테이블 
		모든 ROW에 적용
		{FOR EACH ROW}
		BEGIN
			트리거 실행 문장 => 입고
	...
	시점)
		BEFORE : DML 실행 전에 동작
			=> 발생한 테이블 저장 전에 trigger 수행
		AFTER : DML 실행 후에 동작
			=> 발생한 테이블 저장 후에 trigger 수행

		행 단위 / 문장 단위
			각 행마다 실행 : FOR EACH ROW (default)
			SQL문 전체에서 한 번만 실행 : 생략
		...
		컬럼
			...
			예)
				입고 (상품번호, 수량, 단가)
				=> INSERT INTO 입고 VALUES(새로운 값 : 1, 5, 1500)
				=> :NEW.수량
*/
/*
-- emp 
