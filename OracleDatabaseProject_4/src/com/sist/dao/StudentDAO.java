	...
	public void stdInsert(StudentVO vo)
	{
		...
		try {
			...
			String sql="INSERT INTO student(hakbun,name,kor,eng,math) "
					...
					+"?,?,?,?)"; // ;은 자동 추가
			// SQL Injection 해결 => ?에 값 채우는 방식을 사용
			// PriparedStatement => 미리 SQL을 전송 => 나중에 값을 채워서 실행 
			ps=conn.prepareStatement(sql);
			...
			// 실행 요청
			ps.executeUpdate(); // => insert / update / delete (commit 있음)
//			ps.executeQuery(); // => select (commit 없음)
		}catch(Exception ex) {
			ex.printStackTrace();
		}finally {
